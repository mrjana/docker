...
func (s *DockerSuite) TestApiNetworkInspect(c *check.C) {
	...
	c.Assert(nr.IpamDriver, check.Equals, "default")
	...
	config := types.NetworkCreate{
		...
		Driver:     "bridge",
		IpamDriver: "default",
		Ipam:       []network.IpamData{network.IpamData{Subnet: "172.28.0.0/16", IPRange: "172.28.5.0/24", Gateway: "172.28.5.254"}},
	}
	id0 := createNetwork(c, config, true)
	c.Assert(isNetworkAvailable(c, "br0"), check.Equals, true)

	nr = getNetworkResource(c, id0)
	c.Assert(len(nr.Ipam), check.Equals, 1)
	c.Assert(nr.Ipam[0].Subnet, check.Equals, "172.28.0.0/16")
	c.Assert(nr.Ipam[0].IPRange, check.Equals, "172.28.5.0/24")
	c.Assert(nr.Ipam[0].Gateway, check.Equals, "172.28.5.254")
	// delete the network and make sure it is deleted
...
func (s *DockerSuite) TestApiNetworkIpamMultipleBridgeNetworks(c *check.C) {
	...
	config0 := types.NetworkCreate{
		...
		Driver:     "bridge",
		IpamDriver: "default",
		Ipam:       []network.IpamData{network.IpamData{Subnet: "192.178.0.0/16", IPRange: "192.178.128.0/17", Gateway: "192.178.138.100"}},
	}
	...
	config1 := types.NetworkCreate{
		...
		Driver:     "bridge",
		IpamDriver: "default",
		Ipam:       []network.IpamData{network.IpamData{Subnet: "192.178.128.0/17", Gateway: "192.178.128.1"}},
	}
	...
	config2 := types.NetworkCreate{
		...
		Driver:     "bridge",
		IpamDriver: "default",
		Ipam:       []network.IpamData{network.IpamData{Subnet: "192.169.0.0/16", Gateway: "192.169.100.100"}},
	}
